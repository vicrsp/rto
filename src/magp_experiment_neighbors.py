...
def run_rto_exact(neighbors):
    # Exact algorithm
    exp_name = 'ma-gp-slsqp-{}'.format(neighbors)
    for i in range(n_experiments):
        ...
        adaptation = MAGaussianProcesses(model, initial_data, neighbors)
...
if __name__ == '__main__':
    ...
    p1 = multiprocessing.Process(target=run_rto_exact, args=('k_nearest',))
    p1.start()
    #p2 = multiprocessing.Process(target=run_rto_exact, args=('k_nearest',))
    #p2.start()
