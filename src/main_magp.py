...
def run_rto_exact():
    ...
    for i in range(n_experiments):
        ...
        rto = RTO(model, plant, opt_problem, adaptation, iterations=n_iterations,
                  db_file='/mnt/d/rto_data/rto_poc_exact_slsqp.db', name='ma-gp-slsqp_scipy')
        rto.run(u_0_feas)

def run_rto_exact_trustcon():
    # Exact algorithm
    for i in range(n_experiments):
        print('trustcon_scipy experiment {}'.format(i))
        initial_data = generate_samples_uniform(
            model, plant, g_plant, u_0, data_size)
        opt_problem = BatchProfileOptimizer(
            ub=[30, 0.002, 250], lb=[0, 0, 200], g=[0.025, 0.15], solver='trustcon_scipy')

        adaptation = MAGaussianProcesses(model, initial_data)
        u_0_feas = initial_data[0][0]

        rto = RTO(model, plant, opt_problem, adaptation, iterations=n_iterations,
                  db_file='/mnt/d/rto_data/rto_poc_exact_trustcon.db', name='ma-gp-trustcon_scipy')
        rto.run(u_0_feas)
...
def run_rto_de_best1bin():
    ...
    for i in range(n_experiments):
        ...
        rto = RTO(model, plant, opt_problem, adaptation, iterations=n_iterations,
                  db_file='/mnt/d/rto_data/rto_poc_debest1bin.db', name='ma-gp-de_scipy_best1bin')
        rto.run(u_0)
...
def run_rto_de_rand1bin():
    ...
    for i in range(n_experiments):
        ...
        rto = RTO(model, plant, opt_problem, adaptation, iterations=n_iterations,
                  db_file='/mnt/d/rto_data/rto_poc_derand1bin.db', name='ma-gp-de_scipy_rand1bin')
        rto.run(u_0)
...
if __name__ == '__main__':
    ...
    #p3.start()
    #p4 = multiprocessing.Process(target=run_rto_exact_trustcon)
    #p4.start()
    #p4.join()
    #jobs = [p1, p2, p3, 4]
