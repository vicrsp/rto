...
class ExperimentResultsHandler:
    ...
    def save_models(self, run_id, models_dict):
        self.md.save_models(run_id, models_dict)

    def save_results(self, rto_id, iteration, fr, gr, fm, gm, f_input, opt_feasible, opt_time, n_fev, run_type='closed-loop'):
        run_id = self.md.create_run(rto_id, iteration, run_type)
        ...
        logging.debug(f'[{self.name}]: iteration={iteration}; cost_real={fr}')

        return run_id
    
    def save_run_results(self, run_id, results):
        self.md.save_results(run_id, results)
        return run_id