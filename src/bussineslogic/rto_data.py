...
class RTOData:
    ...
    def create_run(self, rto_id, iteration='0', status='completed'):
    ...
    def save_samples(self, run_id, samples):
        ...
        for time, sample in samples.items():
            for i in range(len(sample)):
                run_samples.append((run_id, time, 'sample_{}'.format(i), sample[i], 1))

        cur.executemany('INSERT INTO sample_values VALUES (?,?,?,?,?)', samples)
        self.conn.commit()
    ...
    def save_parameters(self, run_id, parameters):
        ...
        cur.executemany('INSERT INTO parameter_values VALUES (?,?,?,?)', run_params)
        self.conn.commit()
    ...
    def save_input_data(self, run_id, input_data):
        ...
        cur.executemany('INSERT INTO input_data_values VALUES (?,?,?)', run_params)
        self.conn.commit()
    ...
    def save_results(self, run_id, result_data):
        ...
        cur.executemany('INSERT INTO result_variable_values VALUES (?,?,?)', run_params)
        self.conn.commit()
