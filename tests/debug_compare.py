...
def run_rto_magptr(n_experiments, n_iterations, data_array, solver, db_file, neighbors, exp_name, noise, backoff):
...
def run_rto_magp(n_experiments, n_iterations, data_array, solver, db_file, neighbors, exp_name, noise, backoff):
    for i in range(n_experiments):
        print('{} experiment {}'.format(exp_name, i))
        initial_data = data_array[i]
        # build the model-based optimization problem
...
targets = [run_rto_magp, run_rto_magptr]
...
for target in targets:
    for key, cfg in configs.items():
        ...
        exp_name = f'{target.__name__}-{name}-{neighbors}-{noise}-{backoff}'
